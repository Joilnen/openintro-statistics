...
Outro problema comum é uma \term{amostra conveniente}\index{sample!convenience sample}, onde idivíduos que são facilmente acessíveis são mais frequentemente incluídos na amostra. Portanto, se uma amostra sobre politica é feita parando pedestres no Bronx, isto não irá representar toda a cidade de Nova York. Isto é frequentemente difícil de dicernir que parte da população produz uma amostra representativa.
...
Considere a questão seguinte de page~\pageref{fedSpendingPovertyQuestion}  para os dados\data{county} data set:
...
Se suspeitamos que a pobreza afeta o gasto no município, entáo a pobreza é uma \term{explanatoria} e gasto federal como variável \term{resposta} no relacionamente.\footnote{Algumas vezes uma variável explanatória é chamada \term{indepdendete} e a variável resposta, \term{dependente}. De qualquer forma, isto fica confuso desde que o \emph{par} de variáveis pode ser dependente ou independente, então evitaremos estes termos.} Se há muitas variáveis provavlemnte um número delas devam ser consideradas como explanatórias.
...
Se calculássemos simplesmente a média da variável \var{renda}, trataríamos municípios com 5,000 e 5,000,000 habitantes de forma uniforma nos cálculos.
...
Por exemplo, no conjunto de dados \data{email50}, criamos uma tabela de contagens para o número de casos em que a contagem de números fica entre 0 e 5,000, depois o número de casos entre 5,000 e 10,000, e assim vai.
...
Estas contagens armazenadas são traçadas como barras em figura~\ref{email50NumCharHist} em que se chama um \term{histograma}, o qual se assemelha o traçado de pontos empilhado em figura~\ref{emailCharactersDotPlotStacked}.
...
Count & 19 & 12 & 6 & 2 & 3 & 5 & $\cdots$ & 0 & 1 \\
  \hline
\end{tabular}
\caption{As contagens para os dados \var{num\_\hspace{0.3mm}char} armazenados.}
...
\begin{figure}[bth]
   ...
   \caption{Um histograma de \var{num\_\hspace{0.3mm}char}. Esta distribuição é fortemente enviesado à direita.\index{skew!example: very strong}}
   \label{email50NumCharHist}
\end{figure}

Histogramas proporcionam uma vista da \term{sensibilidade dos dados}.
...
Quando dados diminuem à direita nesta forma e têm à direita um rabo mais cumprido \hiddenterm{tail}\index{skew!tail}, a forma é chamada \termsub{enviesada à direita}{skew!right skewed}.
\footnote{Outras maneiras para descrever dados enviesadas para a direita:
\termni{enviesados à direita}, \termni{enviesados à ponta alta}, or \termni{enviesados à ponta positiva}.}

Conjuntos de dados com a característica inversa -- um rabo cumprido, fino à esquerda -- chamam-se \termsub{enviesadas à esquerda}{skew!left skewed}.
Dizemos também que uma tal distribuição tem um rabo cumprido à esquerda.
Conjuntos de dados que mostram mais ou menos uma diminuição igual em ambos sentidos chamam-se \term{simétricos}.\index{skew!symmetric}

\begin{termBox}{\tBoxTitle{Rabos cumpridos para identificar enviesado}%
 Quando dados diminuem em uma direção, a distribuição tem um \term{rabo cumprido}.
  \index{skew!long tail|textbf}
...
Um \term{modo} é representado por um pico conspícuo na distribuição.
...
Só existe um pico conspícuo no histograma de \var{num\_\hspace{0.3mm}char}.

Figura~\ref{singleBiMultiModalPlots} mostra histogramas que têm um, dois, ou três pico conspícuos.
Tais distribuições chamam-se \termsub{unimodais}{modality!unimodal}, \termsub{bimodais}{modality!bimodal}, e \termsub{multimodais}{modality!multimodal}, respetivamente.
